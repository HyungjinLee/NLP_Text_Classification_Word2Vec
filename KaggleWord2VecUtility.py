...
class KaggleWord2VecUtility(object):
    ...
    @staticmethod
    def review_to_wordlist_with_tag(review, remove_stopwords=False):
        # 1. HTML 제거
        review_text = BeautifulSoup(review, "html.parser").get_text()
        # 2. 특수문자를 공백으로 바꿔줌
        review_text = re.sub('[^a-zA-Z]', ' ', review_text)
        ...
        # 품사 태깅
        tagged=[]
        tagged = pos_tag(review_text.lower().split())
         
        formed_list = []
        stemmer = SnowballStemmer('english')
        
        for tagged_token in tagged :
            tagged_word = tagged_token[0]
            tagged_pos = tagged_token[1]
            tagged_word = stemmer.stem(tagged_word)
            tagged_word = tagged_word + '/' + tagged_pos
            formed_list.append(tagged_word)
        ...
        for raw_sentence in raw_sentences:
            ...
            if len(raw_sentence) > 0:
                ...
                for tagged_word in tagged :
                    formed_list.append(tagged_word[0] + '/' + tagged_word[1])
                sentences.append(formed_list)
